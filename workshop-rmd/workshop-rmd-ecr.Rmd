...
author: "Early Career Researchers"
output:
  html_document:
    toc: true
    toc_depth: 2
...
In order to follow this section, install the libraries 
`ggpubr`, `rstatix` and `corrr`:
...
> 2. Create boxplots and qqplots to test the assumptions of the statistical models; 
> 3. Adjust `ggplot2` objects.\
...
1.  First, we may be interested in getting some summary statistics
    before any statistical testing. Here we use `get_summary_stats()`
    rather than `summarise()` as we did earlier in the workshop.

```{r summary statistics}

flower_data %>% group_by(variety) %>%
...
setosa_versicolor_data <- flower_data %>%
  filter(variety != "Virginica")

```

Create a boxplot using the `ggplot()` function previous introduced.

1.  Include `setosa_versicolor_data` as our `<DATA>;`
2.  Allocate the variable `variety` to the x axis and sepal.length to
    the y axis inside `aes();`
3.  Use the scatterplot function at `<GEOM_FUNCTION>`, which is
    `geom_boxplot()`;
4.  Optional: Use the function `geom_dotplot()` to visualise each
    datapoints;

```{r}

bxp <- ggplot(setosa_versicolor_data, aes(x=variety, y=sepal.length)) + 
  geom_boxplot()+
...
To create a qqplot of `sepal.length` for each `variety` in our
`setosa_versicolor_data`, we:

1.  Use the `ggqqplot()` function from the `ggpubr` library;

2.  Include `setosa_versicolor_data` as our `<DATA>`;

3.  Specify the mappings `x = "sepal.length"` and facet.by =
    "`variety"`;
...
We can further confirm the normality of our distribution by running a
test of normality using the `shapiro_test`from the`rstatix` library.

If the data is normally distributed, or approximates a normal
distribution, the Shapiro-Wilk test should be greater than .05.

```{r}
setosa_versicolor_data %>%
  group_by(variety) %>%
...
### **Homogeneity of variances**: the variance of the outcome variable should be similar in each group;

Similarly to the `shapiro_test`, we can test whether both groups show
similar variance by using the `levene_test` function from `rstatix`
library.

The Levene's test should be non-significant (greater than .05) if the
variances of both groups are similar.

```{r}
setosa_versicolor_data %>%
  levene_test(sepal.length~variety)
```
...
## Fitting an independent t-test()

Since the data meets the assumptions of independence, normality and does
not show extreme outliers we will proceed to run an independent t-test.
First, we will fit the independent t-test with the `t_test()`command.
Within the command, the model is denoted by
`formula = outcome variable ~ explanatory variable`. Second, since the
assumption of homogeneity of variance was not met, we will need to set
`var.equal` to false. Final, given that we are running an independent
t-test, we need to set `paired` to false.

```{r t.test general format}
t.test <- setosa_versicolor_data %>%
  t_test(sepal.length ~ variety, var.equal = FALSE, paired = FALSE)
...
If we wanted to extend the model we just fitted to one-sample t-test to
check whether the petal length for the Versicolor variety is different
than a certain value, we would first have to subset our data. And then
proceed to fit the model as we have already established that the
assumptions for a one-sample t-test are met. One-sample t-tests are
fitted using the same command as before `t_test`, yet the formula is
denoted by `outcome variable ~ 1, mu = parameter.`

In this example we want to check whether the petal length of the
Versicolor variety is significantly different from 3 cm.

```{r}

Versicolor <- flower_data %>%
  filter(variety == "Versicolor")

Versicolor %>%
  t_test(petal.length ~ 1, mu = 3)

```

### Report

A one-sample t-test was run to determine the petal length of the
Versicolor variety is different from 3 cm. The one-sample t-test
revealed that petal length size was statistically significant difference
from 3 cm, *t*(49) = -62.62, *p* \< .001.
...
`petal.length` of `Iris Setosa` are statistically different from 5 cm.
...
virginica <- flower_data %>%
      filter(variety == "Virginica")
    
```

Then we need to inspect whether there is a linear relationship between
...
Finally, given that the relationship appears to be linear, we can now
compute a Pearson's correlation with the `correlate()` command from the
`corrr` library. Within the command, you need to assign the variables of
interest to x and y, the order does not matter.

```{r}

correlate(x = virginica$sepal.length, y = virginica$sepal.width)
...
`sepal_length` and `sepal_width` for the variety of Iris Virginica.
...
using the `health_data`. Then visualise the association using a scatterplot.
...
correlate(health_data$Age, health_data$Weight)
