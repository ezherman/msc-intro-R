...
## Viewing a `tibble`
We will cover three ways to quickly inspect the `tibble` that we have just created. 
...
> 5. Describe the difference between a string variable and a factor variable. 
> [*if there is time*]  
> 6. Use `factor()` to convert a string variable to a character variable. 
> [*if there is time*]  
