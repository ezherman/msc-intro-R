...
We can see that `health_data` has appeared in the *environment* panel. This
...
just created. Firstly, we can view `health_data` as a whole, analogous to
...
sex from `health_data`, then the criterion is `Sex == "female"` (note the
use of the double `=`):
...
Note that in the `filter()` step, we specify `health_data_no_weight` as the data,
rather than `health_data`. So here we `select()` for columns and then
...
**Challenge**: Create an object named `flower_data_setosa_sepal`, a subset of
`flower_data`, with:
...
`health_data` object, based on the `Height` and `Weight` columns. The BMI
is calculated by dividing `Weight` by `Height` squared. Since `Height`
in `health_data` is in cm, we divide `Height` by 100. We create the `BMI`
column using `mutate()`:
...
```{r export health_data summary table, warning = FALSE}
...
write_csv(height_summary, "height_summary.csv")
```
...
object `sepal_width_summary` and save it as a `.csv` file in your working
...
write_csv(sepal_width_summary, "sepal_width_summary.csv")
```
...
-   `ggsave()` requires a file name, such as `"height_weight_plot.png"`. The
...
-   `ggsave()` uses the file extension given in the file name. For
    example, `"height_weight_plot.png"` will produce a `.png` file, while
    `"height_weight_plot.pdf"` will produce a `.pdf` file.
...
ggsave("height_weight_plot.png")
```

We should then have the file `"height_weight_plot.png"` in our project
working directory.
...
`sepal.length` across `variety` from the `flower_data` to create a violin
...
as a `character` column. See for example the `Sex` column in health_data:
...
graphs of data. For example, a boxplot of Height by Sex using `health_data_factor`
...
In contrast, the same code on `health_data_factor_relevel` returns a graph 
...
```{r save sepal length plots, eval = TRUE, fig.alt="A composite figure consisting of two graphs. The left graph is a boxplot with sepal length on the y-axis and variety on the x-axis. The right graph is a violin plot, also with sepal length on the y-axis and variety on the x-axis."}
ggsave("sepal_length_plots.pdf")
```
