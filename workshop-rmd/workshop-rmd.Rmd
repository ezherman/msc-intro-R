...
RStudio *projects* functionality. Here we will set up an RStudio project
for this workshop:
...
-   Provide a *Directory name* - this will be the name of the newly
    created folder. For example, you could name it `msc-intro-R`.
...
four-panel view is restored. Then press on the *Save* icon to the right
of the *New File* button. Give the file a convenient name, such as
`msc-intro-R`. It will then appear in the bottom right of RStudio as
`msc-intro-R.R`.
...
1. Download the data by clicking [here](https://github.com/ezherman/msc-intro-R/raw/main/data.zip).
2. Unzip the folder, and save the data folder in the `msc-intro-R` folder created earlier.
...
exemplified below. If we wanted to save the number 6 under the name
`Six`, we would run:
...
piece of code that performs a specific task. This saves us from having
to write the code required for the task ourselves. When a `function` is
...
Let us take the
`round`()`function as an example.  This function rounds a supplied value.  For example, we`call`the`round()function`on the value 3.14.  The value 3.14 is an`argument`in our`call\`:
...
collection of data points of the *same type* (e.g. numbers of words).
...
interpretability of your code. Find the solution in the drop-down.
...
Working with packages is analogous to using a new light bulb: 1. First,
we install the package in R using the `install.packages()` function.
...
Let us take the `readr` package as an example. In the next section, we
...
whether they need to install any packages before running our script.
...
of a spreadsheet. The main difference is that in `tibbles`, each column
...
> 6. How are string variables converted to factor variables? [*if there
> is time*]\
...
> 5. Describe the difference between a character variable and a factor
> variable. [*if there is time*]\
> 6. Use `as_factor()` to convert a character variable to a factor
> variable. [*if there is time*]\
> 7. Use `drop_na()`, `group_by()`, `summarise()`, `n()`, `mean()` and
...
It is common to provude the `tibble` by itself in the first command,
...
Reordering factor levels will come in handy later on when we learn how
to plot.
...
NAs from `Height` using `drop_na()` from the `tidyr` package. Make sure
...
To do this, we create an object for the sumamry table. Then we export
...
> help of Google.\
> 4. Use the `patchwork` package to group multiple `ggplot()` graphs
> together.\
...
2.  Include `healthData` as out `<DATA>`;
...
`ggplot2` is to consult Google. In this section we will exemplify this
...
Following the tutorial on "sthda.com", which is common source for R
...
Trying this with out plot gives:
...
For example, to save our figure that combined `Height` vs `Weight` and
`Height` vs `Age`, as a `.png` file, we run:

```{r save patchwork healthData part 1, eval = FALSE}
p1 + p2
```

```{r save patchwork healthData part 2, warning = FALSE, message = FALSE}
ggsave("healthDataPlot.png")
```

We should then have the file `"healthDataPlot.png"` in our project
working directory.
...
```{r patchwork and save sepal length plots, message = FALSE}
