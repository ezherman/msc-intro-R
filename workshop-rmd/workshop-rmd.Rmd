...
-   `ggsave()` requires a file name, such as `"heightVsWeightPlot.png"`. The
...
-   `ggsave()` uses the file extension given in the file name. For
    example, `"heightVsWeightPlot.png"` will produce a `.png` file, while
    `"heightVsWeightPlot.pdf"` will produce a `.pdf` file.
...
plot. Finally, save this plot as a `.pdf` file. Find the solution in the drop-down.
...
```{r ggsave violin plot}
# To export to pdf, we use the .pdf file extension
ggsave("sepal_length_by_variety_violin_plot.pdf")
```
...
```{r boxplot healthDataFactor, warning=FALSE, fig.alt="A boxplot with Height on the y-axis and Sex on the x-axis. From left to right, the boxplots are in the following order: male, female."}
...
```{r boxplot healthDataFactorRelevel, warning=FALSE, fig.alt="A boxplot with Height on the y-axis and Sex on the x-axis. From left to right, the boxplots are in the following order: female, male."}
